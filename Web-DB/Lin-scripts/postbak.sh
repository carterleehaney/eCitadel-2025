...
# Function to check if PostgreSQL client (`pg_dump`) is installed
check_postgresql_client() {
    if ! command -v pg_dump &>/dev/null; then
        echo "Error: pg_dump not found. Please install PostgreSQL client before running this script."
        exit 1
    fi
}
...
# Dump the PostgreSQL database (password is passed securely)
PGPASSWORD="$DB_PASSWORD" pg_dump -U "$DB_USER" --no-password --format=c --blobs --file="$BACKUP_FILE"
