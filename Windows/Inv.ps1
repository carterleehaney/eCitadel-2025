...
if ($DC) {
    ...
    try {
        ...
        try {
            $dnsRecords = Get-WmiObject -Namespace "root\MicrosoftDNS" -Class "MicrosoftDNS_AType" | Select-Object OwnerName, RecordData
            $dnsRecords | Format-Table -AutoSize
        }
        catch {
            Write-Output "[ERROR] Failed to get DNS records using WMI"
        }
    }
...
if ($DC) {
    if (-not (Get-Command Get-ADGroup -ErrorAction SilentlyContinue)) {
        $Groups = Get-WmiObject Win32_Group | Select-Object -ExpandProperty Name | Sort-Object | Get-Unique
        $Groups | ForEach-Object {
            $GroupObj = $_
            $Users = Get-WmiObject Win32_GroupUser | Where-Object { $_.GroupComponent -match $GroupObj } | Select-Object -ExpandProperty PartComponent | ForEach-Object { $_.Split(",")[1].Split("=")[1].Replace('"', '') }
            if ($Users.Count -gt 0) {
                $Users = $Users| Sort-Object | Get-Unique | ForEach-Object { "   Member: $_" }
                Write-Output "Group: $GroupObj" $Users
            }
        }
    } else {
